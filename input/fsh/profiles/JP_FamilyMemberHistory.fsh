...
* dataAbsentReason ^definition = "Describes why the family member's history is not available.\r\n\r\n1)subject-unknown:患者が対象の家族のことを知らない（例:患者は養子のため実親を知らない）, 2)withheld:患者が家族歴の共有を拒否, 3)unable-to-obtain:患者から聞き取れない（例：患者が意識不明）, 4)deferred:患者は今は知らないが、後日共有するつもりである。"
