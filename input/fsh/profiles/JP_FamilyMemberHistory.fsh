...
* . ^short = "同胞内出生順"
* . ^definition = "FamilyMemberHistoryのrelationship（続柄）と組み合わせて、関連する家族構成員の同胞内出生順と性別同胞内出生順を表現する拡張。同胞内の順位を整数で、性別の同胞内の出生準の名称（長男、長女、次男、次女など）を CodeableConcept で表現する。特に、ジェノグラム（家族図）のように社会的・文化的文脈で利用される。また、日常の臨床では家族の氏名が聴取されずに続柄で記述され、家族全員の生年月日や年齢を聴取できない場合が多いことを想定している。遺伝学的血統図は対象外である。本実装ガイド（Version 1.2.0）の範囲外であるが、 参考としてGenetic Pedigreeプロファイルを参照すること。"
