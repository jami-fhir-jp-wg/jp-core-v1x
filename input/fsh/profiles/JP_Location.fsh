...
* identifier ^short = "ユーザーに場所を特定する一意のコードまたは番号"
* identifier ^definition = "ユーザーに場所を特定する一意のコードまたは番号"
...
* type ^comment = "すべての用語の使用がこの一般的なパターンに適合するわけではない。場合によっては、モデルはCodeableConceptを使用せず、`Coding`を直接使用して、テキスト、`Coding`、翻訳、および要素と事前・事後の用語結合の関係を管理するための独自の構造を提供する必要がある。"
...
* physicalType ^comment = "すべての用語の使用がこの一般的なパターンに適合するわけではない。いくつかのケースでは、モデルはCodeableConceptを使用せず、`Coding`を直接使用し、テキスト、`Coding`、翻訳、要素間の関係、および事前・事後の用語結合を管理するための独自の構造を提供することが望ましい。"
...
* hoursOfOperation.daysOfWeek ^comment = ""
