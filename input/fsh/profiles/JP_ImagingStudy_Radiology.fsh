...
* status ^comment = "不明は「その他」を表すものではない。定義されたステータスの1つを適用する必要がある。不明は、オーサリングシステムが現在のステータスを確認できない場合に使用される。\r\n\r\n【JP-Core仕様】リソースの状態。"
* modality ^short = "実際の取得モダリティーの場合、モダリティーの全シリーズ"
...
* endpoint ^definition = "スタディへのアクセス（クエリ、表示、取得など）を提供するネットワークサービス。 DICOMエンドポイントの使用については、実装ノートを参照。スタディレベルのエンドポイントは、同じEndpoint.connectionType.cを持つシリーズレベルのエンドポイントによってオーバーライドされない限り、スタディの各シリーズに適用される。"
...
* series.endpoint ^definition = "シリーズへのアクセス（クエリ、表示、取得など）を提供するネットワークサービス。 DICOMエンドポイントの使用については、実装ノートを参照。シリーズレベルのエンドポイントは、同じEndpoint.connectionType.cを持つシリーズレベルのエンドポイントによってオーバーライドされない限り、スタディの各シリーズに適用される。"
...
* series.bodySite ^definition = "検査対象の解剖学的構造。 DICOMからSNOMED-CTへのマッピングについては、DICOM Part 16 Annex L（https://dicom.nema.org/medical/dicom/current/output/chtml/part16/chapter_L.html）を参照。\r\nbodySiteは、画像化された身体部分の左右差を示している場合がある。その場合、ImagingStudy.series.lateralityのコンテンツと一致している必要がある。"
* series.bodySite ^comment = "コードは、列挙型またはコードリストで、SNOMED CTなどの非常に正式な定義まで、非常に柔軟に定義できる。ただしJP CoreではSNOMED CTを推奨しない。詳細については、HL7v3コア原則を参照。\r\n\r\nどの部位の検査なのかを示す。フリーではなく、DICOM定義書の中で示される語句しか使えない。"
...
* series.laterality ^comment = "コードは、列挙型またはコードリストで、SNOMED CTなどの非常に正式な定義まで、非常に柔軟に定義できる。ただしJP CoreではSNOMED CTを推奨しない。詳細については、HL7v3コア原則を参照。\r\n\r\nJP Coreでは左右などを示す。フリーではなく、DICOM定義書の中で示される語句しか使えない。"
...
* series.performer ^definition = "シリーズを実施し、かかわった人を示す。"
* series.performer ^comment = "シリーズを行った人がわからない場合は、その組織が記録される場合がある。患者または関係者は、患者自身で取得した画像の場合など、パフォーマーである可能性がある。\r\n\r\n【JP Core仕様】検査を実施した（画像を取得する操作を行った）人。\r\n\r\n　　(0008,1050)Performing Physician's Name\r\n\r\n　　(0008,1052)Performing Physician Identification Sequence\r\n\r\n　　(0008,1070)Operators' Name\r\n\r\n　　(0008,1072)Operator Identification Sequence"
