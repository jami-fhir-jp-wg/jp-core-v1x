...
* dosageInstruction.additionalInstruction ^short = "患者に対する補足指示や注意。たとえば、「食事と一緒に」「眠くなることがある」"
* dosageInstruction.additionalInstruction ^definition = "どのように薬剤を内服するかについての患者への補足指示（たとえば、「食事と一緒に」や「30分から1時間前に内服」）や薬剤についての注意書き（たとえば、「眠くなることがある」や「日光や紫外線が皮膚にあたららないようにすること」）"
...
* dosageInstruction.timing.code ^definition = "スケジュール上のタイミングを表すコード（あるいはcode.text内のテキスト）。BID(1日2回)のようなコードはどこにでもあるが、多くの医療機関は付加的なコードを定義している。もし、コードが示されていれば、構造化されたタイミングで完全に示されたデータであると解釈され、コードまたはTimingを解釈するためのデータであると解釈される。しかし、例外的に.repeat.bounds（コードは含まれない)はコードを上書きして適用される。"
...
* dosageInstruction.route.coding.system ^short = "HL7v2-0162表をベースにしたJP Coreでの投与経路コード表"
* dosageInstruction.route.coding.system ^definition = "HL7v2-0162表をベースにしたJP Coreでの投与経路コード表を示すURL"
...
* dosageInstruction.maxDosePerPeriod.numerator.system = "urn:oid:1.2.392.100495.20.2.101"
...
* dosageInstruction.maxDosePerPeriod.denominator.system = "urn:oid:1.2.392.100495.20.2.101"
...
* dosageInstruction.maxDosePerAdministration.system = "urn:oid:1.2.392.100495.20.2.101"
...
* dosageInstruction.maxDosePerLifetime.system = "urn:oid:1.2.392.100495.20.2.101"
...
* dispenseRequest.initialFill.quantity.system = "urn:oid:1.2.392.100495.20.2.101"
