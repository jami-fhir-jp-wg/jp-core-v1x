...
* dosageInstruction.doseAndRate.rateRatio ^short = "単位時間内での薬剤の容量（体積）"
* dosageInstruction.doseAndRate.rateRatio ^comment = "【JP Core仕様】単位時間内での薬剤の容量（体積）、すなわち投与速度を記述する。単位はUCUMを使用する。"
* dosageInstruction.doseAndRate.rateRatio ^requirements = "患者の体内に導入される、あるいはされた薬剤の速度を指定する。一般的には、たとえば1時間あたり100mlあるいは100ml/hrのように注射の速度を示す。たとえば、500mlを2時間でというように、単位時間あたりの速さを表現することもできる。その他、200マイクログラム/minや200マイクログラム/1分, 1 リットル/8時間のような表現もできる。しばしば、投与速度を投与総量/ 投与総時間で表ような場合に投与時間が明示される（たとえば、500ml/2時間という場合は、投与時間が2時間であることを示している）。しかしながら、投与速度で投与時間が明示されない場合（たとえば、250ml/毎時)は、timing.repeat.durationが注射の総投与時間を示すためには必要となる。"
* dosageInstruction.doseAndRate.rateRatio.numerator ^short = "投与量（体積）"
* dosageInstruction.doseAndRate.rateRatio.numerator ^definition = "投与量（体積）"
* dosageInstruction.doseAndRate.rateRatio.numerator.value ^short = "投与量（体積）"
* dosageInstruction.doseAndRate.rateRatio.numerator.value ^definition = "投与量（体積）"
* dosageInstruction.doseAndRate.rateRatio.numerator.system = "http://unitsofmeasure.org" (exactly)
* dosageInstruction.doseAndRate.rateRatio.numerator.unit ^short = "投与量（体積）の単位"
* dosageInstruction.doseAndRate.rateRatio.numerator.unit ^definition = "投与量（体積）の単位。UCUMを使用する。"
* dosageInstruction.doseAndRate.rateRatio.denominator ^short = "単位時間"
* dosageInstruction.doseAndRate.rateRatio.denominator ^definition = "単位時間"
* dosageInstruction.doseAndRate.rateRatio.denominator.value ^short = "単位時間"
* dosageInstruction.doseAndRate.rateRatio.denominator.value ^definition = "単位時間"
* dosageInstruction.doseAndRate.rateRatio.denominator.unit ^short = "単位時間の単位"
* dosageInstruction.doseAndRate.rateRatio.denominator.unit ^definition = "単位時間の単位。UCUMを使用する。"
* dosageInstruction.doseAndRate.rateRatio.denominator.system = "http://unitsofmeasure.org" (exactly)
* dosageInstruction.doseAndRate.rateRatio.denominator.system ^short = "UCUMを識別するURL。"
* dosageInstruction.doseAndRate.rateRatio.denominator.system ^definition = "UCUMを識別するURL。固定値。"
