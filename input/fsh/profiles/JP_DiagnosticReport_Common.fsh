...
* encounter ^short = "依頼時におけるヘルスケアイベント（受診など）"
...
* effective[x] ^short = "臨床的に関連する時刻または時間"
...
* performer ^short = "レポート内容に責任をもつ診断的サービス"
...
* result ^short = "検査結果。Observationsリソースが参照される。"
...
* imagingStudy ^short = "診断レポートに関連づけれらた画像検査の詳細情報への参照"
...
* presentedForm ^short = "発行されたレポート全体"
