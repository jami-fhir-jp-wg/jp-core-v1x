...
* basedOn ^short = "What was requested　元になった検査や診断の依頼"
...
* category ^short = "Service category　サービスカテゴリー"
* category ^definition = "A code that classifies the clinical discipline, department or diagnostic service that created the report (e.g. cardiology, biochemistry, hematology, MRI). This is used for searching, sorting and display purposes.\r\n\r\nレポートを作成した臨床分野、部門、または診断サービス（心臓病学、生化学、血液学、MRIなど）を分類するコード。これは、検索、並べ替え、および表示の目的で使用される。"
...
* code ^short = "Name/Code for this diagnostic report　この診断レポートの名前/コード"
...
* subject ^short = "The subject of the report - usually, but not always, the patient 　レポートの対象、常にではありませんが、通常は患者"
...
* encounter ^short = "Health care event when test ordered　依頼時におけるヘルスケアイベント（受診など）"
...
* effective[x] ^short = "Clinically relevant time/time-period for report　臨床的に関連する時刻または時間"
* effective[x] ^definition = "The time or time-period the observed values are related to. When the subject of the report is a patient, this is usually either the time of the procedure or of specimen collection(s), but very often the source of the date/time is not known, only the date/time itself.\r\n\r\n観測値が関連する時間または期間。レポートの対象が患者である場合、これは通常、処置や検体採取の時刻のいずれかですが、日付/時刻の出処は不明であり、日付/時刻自体のみのことがある。"
...
* issued ^short = "DateTime this version was made　このバージョンが作成された日時"
...
* performer ^short = "Responsible Diagnostic Service　レポート内容に責任をもつ診断的サービス"
* performer ^definition = "The diagnostic service that is responsible for issuing the report.\r\n\r\nレポートの作成発行に責任をもつ診断サービス。"
...
* resultsInterpreter ^short = "Primary result interpreter　結果の一次解釈者"
* resultsInterpreter ^definition = "The practitioner or organization that is responsible for the report's conclusions and interpretations.\r\n\r\nレポートの結論と解釈に責任を持つ医療者あるいは組織。"
...
* specimen ^short = "Specimens this report is based on　レポートのもとになった検体"
...
* imagingStudy ^short = "Reference to full details of imaging associated with the diagnostic report　診断レポートに関連づけれらた画像検査の詳細情報への参照"
* imagingStudy ^definition = "One or more links to full details of any imaging performed during the diagnostic investigation. Typically, this is imaging performed by DICOM enabled modalities, but this is not required. A fully enabled PACS viewer can use this information to provide views of the source images.\r\n\r\n診断的精査中に実施されたなんらかの画像検査の詳細情報へのひとつ以上のリンク。通常は、DICOM対応のモダリティーによって実施される画像検査であるが、必須ではない。完全対応のPACSビューワはこの情報を使用して原画像の一覧を提供できる。"
...
* media ^short = "Key images associated with this report　このレポートに関連づけられたキー画像"
* media ^definition = "A list of key images associated with this report. The images are generally created during the diagnostic process, and may be directly of the patient, or of treated specimens (i.e. slides of interest).\r\n\r\nこのレポートに関連する主要な画像のリスト。画像は通常、診断プロセス中に作成され、患者から直接か、調製された検体標本（つまり、関心のあるスライド）のこともある。"
* conclusion ^short = "Clinical conclusion (interpretation) of test results　検査結果の臨床的結論（解釈）"
...
* conclusionCode ^short = "Codes for the clinical conclusion of test results　検査結果の臨床的結論（解釈）用のコード"
* conclusionCode ^definition = "One or more codes that represent the summary conclusion (interpretation/impression) of the diagnostic report.\r\n\r\n診断レポートの要約的な結論（解釈/印象）を表す1つ以上のコード。"
...
* presentedForm ^short = "Entire report as issued　発行されたレポート全体"
* presentedForm ^definition = "Rich text representation of the entire result as issued by the diagnostic service. Multiple formats are allowed but they SHALL be semantically equivalent.\r\n\r\n診断サービスによって発行された結果全体のリッチテキスト表現。複数のフォーマットが許可されるが、それらは意味的に同等である必要がある。"
