...
* insert SetDefinition(code, 診断レポートを説明するコードまたは名前)
* code ^comment = ""
...
* insert SetDefinition(performer, 診断レポートの作成発行に責任をもつ診断サービス/担当者に関する情報)
...
* insert SetDefinition(resultsInterpreter, 診断レポートの結論と解釈に責任を持つ医療者/組織に関する情報)
...
* insert SetDefinition(imagingStudy, 診断レポートに関連づけれられたDICOM検査画像に関する情報)
...
* insert SetDefinition(presentedForm, 診断サービス/担当者によって発行された診断レポート全体のコンテンツ（例えばPDFファイルなど）)
