...
* insert SetDefinition(category[second],第2カテゴリはLOINCのPartコードLP7796-8（内視鏡）固定とする。ValueSetのバインディングを指定していないことに注意すること。)
* category[second] ^short = "第2カテゴリはLOINCのPartコードLP7796-8（内視鏡）固定とする。ValueSetは指定しない。"
...
* value[x] MS
* value[x] from JP_ObservationEndoscopyValueJed_VS (example)
