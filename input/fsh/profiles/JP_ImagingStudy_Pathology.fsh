...
* ^status = #active
* ^version = "1.0.0"
* ^publisher = "FHIR Japanese Implementation Working Group"
...
* referrer ^short = "依頼医師。必須ではない（ServiceRequestから参照できるため）。"
* referrer ^definition = "依頼医師。必須ではない（ServiceRequestから参照できるため）。"
* interpreter only Reference(JP_Practitioner or JP_PractitionerRole)
* interpreter ^short = "依頼医師。【詳細参照】"
* interpreter ^definition = "依頼医師。"
* interpreter ^comment = "病理では、原則使用しない。"
* endpoint ^short = "画像を診断した医師。【詳細参照】"
* endpoint ^definition = "画像を診断した医師。"
* endpoint ^comment = "通常、病理医。\n病理では原則使用しない。"
* numberOfSeries ^short = "このDICOM画像リソースが存在する位置。【詳細参照】"
* numberOfSeries ^definition = "このDICOM画像リソースが存在する位置。"
* numberOfSeries ^comment = "基本的には1つ指定する。"
* numberOfInstances ^short = "このDICOM画像検査に含まれるシリーズ数。"
* numberOfInstances ^definition = "このDICOM画像検査に含まれるシリーズ数。"
* procedureReference only Reference(JP_Procedure)
* procedureReference ^short = "このDICOM画像に含まれるイメージ（インスタンス）の数。"
* procedureReference ^definition = "このDICOM画像に含まれるイメージ（インスタンス）の数。"
* procedureCode ^short = "実施された処置に関する情報。【詳細参照】"
* procedureCode ^definition = "実施された処置に関する情報。"
* procedureCode ^comment = "病理では省略してよい。\n使用する場合には、JP Core Procedureを参照する。"
* location only Reference(JP_Location)
* location ^short = "実施された処置を表すコード。【詳細参照】"
* location ^definition = "実施された処置を表すコード。"
* location ^comment = "病理では原則使用しない。"
* reasonCode ^short = "検査目的【詳細参照】"
* reasonCode ^definition = "検査目的"
* reasonCode ^comment = "病理では原則使用しない。"
* reasonReference only Reference(JP_Condition or JP_Observation_Common or JP_DiagnosticReport_Common or DocumentReference or Media)
* reasonReference ^short = "DICOM画像が依頼された理由を表す1つ以上のコード。【詳細参照】"
* reasonReference ^definition = "DICOM画像が依頼された理由を表す1つ以上のコード。"
* reasonReference ^comment = "病理では原則使用しない。"
* note ^short = "DICOM画像の実施理由に関する情報。【詳細参照】"
* note ^definition = "DICOM画像の実施理由に関する情報。"
* note ^comment = "JP Core DiagnosticReportリソースを参照する。"
...
* series.modality ^comment = "病理（WSI）を表すモダリティコード\"SM\"を指定する。\n1シリーズ1モダリティで、1つのシリーズの中に複数のモダリティが混在することはない。"
...
* series.specimen only Reference(JP_Specimen_Common)
