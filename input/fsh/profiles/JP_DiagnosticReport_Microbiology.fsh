...
* insert SetDefinition(identifier, 実行者/実施者または他のシステムによってこのレポートに割り当てられた識別子)
* insert SetDefinition(basedOn, 依頼されたサービス（検査や診断など）に関する詳細情報)
...
* category[first].coding.code = $Loinc_CS#LP7819-8 "微生物検査/MICRO"
...
* insert SetDefinition(encounter, 直近の細菌検査に直結するEncounterリソースを参照)
...
* insert SetDefinition(effective[x],  検体採取日時（effectivePeriod は未使用）)
...
* insert SetDefinition(issued,  最新のレポート確定日)
...
* insert SetDefinition(performer, 診断レポートの作成発行に責任をもつ診断サービス/担当者に関する情報、原則未使用とする)
* performer ^comment = "必ずしも１項目データ単位のデータソースまたは結果を解釈した主体でなない。臨床レポートに責任をもつ主体のこと。"
* insert SetDefinition(resultsInterpreter,  検査責任者（外部検査会社への委託の場合には組織名を指定することもある）)
...
* insert SetDefinition(result, 該当するObservationを設定する)
* result ^comment = "Observationsは階層構造を持てる。"
* result ^requirements = "個々の結果または結果のグループをサポートする必要がある。結果のグループ化は任意だが、意味がある場合にグループ化される。"
* insert SetDefinition(imagingStudy, 診断レポートに関連づけれられたDICOM検査画像に関する情報、原則未使用)
...
* insert SetDefinition(conclusionCode, 診断レポートの要約結論（解釈、インプレッション）を表すコード、原則未使用)
* conclusionCode ^comment = "すべてのターミノロジの使用がこの一般的なパターンに適合するわけではない。場合によっては、独自の構造を提供する必要がある。"
* insert SetDefinition(presentedForm, 診断サービス/担当者によって発行された診断レポート全体のコンテンツ（例えばPDFファイルなど）、原則未使用)
