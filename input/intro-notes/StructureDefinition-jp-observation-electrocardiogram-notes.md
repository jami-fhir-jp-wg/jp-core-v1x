...
category.code = "11524-6" 固定値
category.display = "EKG study"
...
リクエスト：ある患者のすべての心電図検査から最新の3件を取得する。

```
GET [base]/Observation/$lastn?max=3&patient=Patient/123&category=procedure&
```
...
### 検査における生理学的に重要な時間
effectiveDateTimeとeffectivePeriodは、検査に密接に関連する時間情報である。 生物学的対象（例：人間の患者）の場合、生理学的に検査に関連する時間である。起床時や就寝前などの時刻や、症状発現時、発作時などの時刻を示す。
