...
#### OID形式からURL形式への移行
FHIR実装ガイドで採用している「CodeSystem」や「Identifierに対するSystem値」に対するURIについて、可能なものからOID形式からURL形式への移行を進めている。URL形式がWebにおける標準的な識別子であり、人間の可読性、相互運用性やアクセシビリティの向上に寄与するためである。

現時点では既存システムとの互換性を維持し円滑な移行を促すため、URL形式の利用は必須ではせずNamingSystemリソースへのOID形式の記載もしてあるOIDについても許容している。しかし、HL7 FHIR標準Validatorやその他のFHIRライブラリはNamingSystemリソースの内容まで参照していないことが多い。そのため実装者は、1)提供されるPackageファイルをNamingSystemリソースを参照し、OID形式への逆変換を行う、2)OIDベースのシステムからデータを外部に出力する際に、URL形式に変換するなどの工夫が必要となる。
