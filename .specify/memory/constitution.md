...
- デフォルト言語は日本語（`ja`）とする
- ドキュメントは常体（断定調）を使用する
- 句読点は「、」と「。」を使用し、全角カンマ「，」・全角ピリオド「．」は
  使用しない（公用文作成の要領に準拠）。番号ラベルや区切りの「．」は
  「、」に、省略記号「．．．」は「…」に置き換える。ただしURL・コード・
  バージョン番号・小数など非文章中の記号はこの限りではない
- カタカナ語末尾の長音記号は、内閣告示「外来語の表記」に従い付与する
  ...
  なお、コードシステムの定義値・表示名（display）およびそれらを利用した
  example のコード値は原典の表記を保持し、本ルールの対象外とする
- 全ドキュメントは日本語で記述可能とし、英語補足は任意とする

#### 用語の使い分け: Coding / コード化

FHIR データ型 `Coding`・コードを付与する行為・実装（プログラミング）
の「コーディング」が同一表記で混在すると誤読を招くため、以下に統一する。

- **FHIR データ型としての `Coding`**: 英語のまま `Coding` と表記する
  （インラインコードまたはコードブロック）。カタカナ「コーディング」を
  用いない。
- **行為（コードを付与すること）**: 「コード化」と表記する。
- **HL7 v2 の「コーディングシステム」等の用語集**: FHIR 用語に合わせ
  「コードシステム」と表記する。
- カタカナ「コーディング」は使用しない。
