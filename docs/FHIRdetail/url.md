...
注：[FHIR仕様の記述](https://www.hl7.org/fhir/r4/http.html#:~:text=Servers%20SHALL%20replace,are%20not%20replaced.)では置換対象としてuuid型とoid型のエレメントを含んでいます。しかし、uuidやoidをurlへ置換するとそれぞれの型の値としては不適当になるため、誤記かもしれません（[HL7要確認](hl7question.md#4. )）。


書き換え前のURLはIDとして扱うため、エンコードの有無は意味を持ちません。（書き換えパターンが偶然他の部分に一致してしまうような書き換えミスを防ぐためにはIDとしてuuidを使うことがのぞましいと思われます。）
...
URLのパーセントエンコードの方法は一意ではないため、検索にヒットさせるためにはエンコード方法を統一しておく必要があります。
...
* **課題**
    ...
    * CanonicalURLにはURLエンコードの不要な文字列を採用すべきと明記するか。
